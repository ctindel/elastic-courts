...
def ingest_to_elasticsearch(record, index_name, es_host):
    ...
    try:
        ...
        # Normalize index name - remove date suffix if present
        normalized_index = index_name.split('-')[0] if '-' in index_name else index_name
        
        # Construct the URL for the Elasticsearch API
        if record_id:
            url = f"{es_host}/{normalized_index}/_doc/{record_id}"
        else:
            url = f"{es_host}/{normalized_index}/_doc"
...
def process_message(message, topic, es_host):
...
def consume_from_topic(topic, es_host, max_workers):
...
def consume_from_retry_topic(retry_topic, es_host, max_workers):
