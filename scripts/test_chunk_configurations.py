...
def test_chunk_configuration(text, chunk_size, chunk_overlap):
...
def run_tests(text, chunk_sizes, chunk_overlaps):
    ...
    for size in chunk_sizes:
        for overlap in chunk_overlaps:
            ...
            result, chunks = test_chunk_configuration(text, size, overlap)
...
def print_summary(results):
    ...
    for result in sorted_results:
        print(f"{result['chunk_size']:<10} {result['chunk_overlap']:<10} {result['chunk_count']:<10} {result['token_counts']['avg']:.2f}{'':>8} {result['processing_time']:.4f}s{'':>6}")
    ...
    efficiency = min(results, key=lambda x: x['processing_time'] / x['chunk_count'] if x['chunk_count'] > 0 else float('inf'))
    ...
    print(f"  Processing time per chunk: {efficiency['processing_time'] / efficiency['chunk_count']:.6f}s")
...
def main():
    ...
    parser.add_argument("--sizes", default="500,1000,1500,2000", help="Comma-separated list of chunk sizes to test")
    parser.add_argument("--overlaps", default="100,200,300", help="Comma-separated list of overlap sizes to test")
    ...
    # Run tests
    results, chunks = run_tests(text, chunk_sizes, chunk_overlaps)
