...
def process_large_file_in_chunks(filename, index_name, chunk_size=100000, batch_size=1000, max_workers=4):
    """Process a large file in chunks to avoid memory issues"""
    print(f"Processing large file {filename} in chunks...")
    total_success = 0
    total_error = 0
    ...
    try:
        with bz2.open(os.path.join(DOWNLOADS_DIR, filename), 'rt', encoding='utf-8') as file:
            ...
            # Process remaining documents
            if chunk:
                print(f"Processing final chunk of {len(chunk)} documents...")
...
def main():
    ...
    for filename in files:
        ...
        if file_size > 1000000000:  # 1GB
            success, error = process_large_file_in_chunks(filename, index_name)
        else:
